  ...
  for($i=1;$i -le $all.length-1;$i++)
  {
   ...
   if($?){
     ...
     set-aduser -identity ($all[$i].split(","))[0] -Department ($all[$i].split(","))[3] -Company ($all[$i].split(","))[4] -Description ($all[$i].split(","))[3] -ChangePasswordAtLogon $false -PasswordNeverExpires $false -enabled $false
   }
}
